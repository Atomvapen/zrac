...
fn drawBox(riskProfile: State, draw_buffer: *DrawBuffer) !void {
    const box: DrawBuffer.Command = .{ .PolyLine = DrawBuffer.Command.create(.PolyLine).init(
        &[_]rl.Vector2{
...
fn drawRisk(riskProfile: State, risk_origin: rl.Vector2, angle: f32, draw_buffer: *DrawBuffer) !void {
    ...
    try draw_buffer.append2(geo.Shape{ .Line = h });
    try draw_buffer.append2(geo.Shape{ .Line = v });
    try draw_buffer.append2(geo.Shape{ .Line = ch });
    try draw_buffer.append2(geo.Shape{ .Line = c });
    try draw_buffer.append2(geo.Shape{ .Line = q });
    try draw_buffer.append2(geo.Shape{ .Semicircle = hv });
    try draw_buffer.append2(geo.Shape{ .Point = Amin });
    try draw_buffer.append2(geo.Shape{ .Point = f });
    try draw_buffer.append2(geo.Shape{ .Point = forestMin });
